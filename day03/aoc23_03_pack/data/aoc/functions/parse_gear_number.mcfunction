...
execute unless block ~ ~ ~ air run fill ~ ~ ~ ~ 0 ~ blue_concrete replace red_concrete
# Next digit is ~1 ~1 ~
execute unless block ~ ~ ~ air positioned ~1 ~1 ~ run function aoc:parse_gear_number
# Multiply number into gear ratio
execute if block ~ ~ ~ air run scoreboard players operation Buffer2 variable *= Buffer variable
# Add 1 to count
execute if block ~ ~ ~ air run scoreboard players add Count variable 1
# Clear buffer
